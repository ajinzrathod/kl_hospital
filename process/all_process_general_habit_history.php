...
if($page == 'view') {
	...
	$result2 = $result = $mysqli->query('
		...
		SELECT * FROM `general_habit_history` 
		WHERE 
		`patient_id` 
		IN (
		SELECT `id`
	...
	while ($row = $result->fetch_assoc()) {
		...
		<tr>
			...
			<td><?php echo $row['eneuresis']; ?></td>
			<td><?php echo $row['thumb_sucking']; ?></td>
			<td><?php echo $row['nightmares']; ?></td>
			<td><?php echo $row['other_habits']; ?></td>
	...
	if ($input['action'] == 'edit') {
		$mysqli->query("
			UPDATE `general_habit_history`
			
			SET

			eneuresis='" . $input['eneuresis'] . "',
			thumb_sucking='" . $input['thumb_sucking'] . "',
			nightmares='" . $input['nightmares'] . "',
			other_habits='" . $input['other_habits'] . "',
			source='" . $input['source'] . "'
